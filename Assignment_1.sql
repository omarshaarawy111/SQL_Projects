...
CREATE TABLE IF NOT EXISTS `customer` (
  ...
  FOREIGN KEY (`AGENT_CODE`) REFERENCES `agents`(`AGENT_NAME`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1;
...
CREATE TABLE IF NOT EXISTS `orders` (
  ...
  primary key (`ORD_NUM`),
  FOREIGN KEY (`AGENT_CODE`) REFERENCES `agents`(`AGENT_NAME`),
  FOREIGN KEY (`CUST_CODE`) REFERENCES `customer`(`CUST_NAME`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1;
...
SELECT * FROM customer WHERE `WORKING_AREA` = 'London';
...
SELECT * FROM customer WHERE `CUST_COUNTRY` = 'USA' OR `CUST_COUNTRY` =  'Australia' OR `CUST_COUNTRY` = 'UK';
SELECT * FROM orders WHERE MONTH(`ORD_DATE`)= '1';
SELECT * FROM customer WHERE `PAYMENT_AMT` BETWEEN 1000 AND 4000;
SELECT customer.*,orders.ORD_NUM
...
ON customer.CUST_CODE =orders.CUST_CODE  
;
SELECT COUNT(`CUST_CODE`) AS CUST_COUNT , `CUST_COUNTRY` FROM customer 
GROUP BY `CUST_COUNTRY`
ORDER BY CUST_COUNT DESC ;
SELECT `AGENT_CODE` ,COUNT(`ORD_NUM`) AS ORD_COUNT FROM orders 
GROUP BY  `AGENT_CODE`
